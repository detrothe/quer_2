...
function menuItemListener(link) {
    ...
    if (link.getAttribute("data-action") === "copyFirst") {
        ...
        for (let i = 0; i < app.npkte; i++) {
            ...
            if (selectedCellPoly.selColY[i]) {
                tabelle.rows[i + 1].cells[1].innerText = wert.toString();
            }
            if (selectedCellPoly.selColZ[i]) {
                tabelle.rows[i + 1].cells[2].innerText = wert.toString();
            }
        }
        ...
        for (let i = 0; i < app.npkte; i++) {
            wertInSpalte1 = false;
            if (selectedCellPoly.selColY[i]) {
                let wert = tabelle.rows[i + 1].cells[1].innerText;
                newClip += wert;
                wertInSpalte1 = true;
            }
            if (selectedCellPoly.selColZ[i]) {
